...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs)
{
    ...
    while (get_current_batch(net) < net.max_batches) {
        ...
        */
        ...
        next_map_calc = fmax(next_map_calc, 400); //ORIGINAL: 400
        if (calc_map) {
...
#endif
            char buff[256];
            sprintf(buff, "%s/%s_%d.weights", backup_directory, base, i);   //ADDITIONAL CODES SO THAT WEIGHTS WILL BE SAVED EVERY 100 EPOCHS 
            save_weights(net, buff);                                        //ADDITIONAL CODES SO THAT WEIGHTS WILL BE SAVED EVERY 100 EPOCHS
